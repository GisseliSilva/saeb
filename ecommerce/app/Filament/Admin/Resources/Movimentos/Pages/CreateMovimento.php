...
class CreateMovimento extends CreateRecord
{
    ...
    protected function beforeCreate(): void
    {
        ...
        if ($tipo === 'saida' && $quantidade> $produto->$estoque){ //-> acessar propriedades e método 
            Notification::make() //make: Criar uma notificação aparecendo em vermelho no topo
                ->title('Estoque insuficiente')
                ->body("Estoque de '{$produto->$nome}' é de apenas {$produto->$estoque} unidades.")
                ->danger() //estilo vermelho
                ->send(); //Enviar a notificação para o usuário no painle admin
            $this->halt(); //execute imediatamente 

        }
    }

    //Criar Hook para diminuir o estoque de produto se der certo
